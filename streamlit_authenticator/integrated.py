...
def stock_analysis():
    ...
    st.sidebar.image("D:/stock_prediction/github_repo/StockStream/Images/StockStreamLogo1.png", width=250,
...
if st.session_state["authentication_status"]:
    ...
    try:
        if authenticator.reset_password(st.session_state["username"], 'Reset password'):
            ...
            st.error(e)
    # Create a text input field for the new password
    ...
    #         try:
    #             if authenticator.reset_password(st.session_state["username"], new_password_hash):
    #                 st.success('Password modified successfully')

    #                 # Update the password hash in the config.yaml file
    #                 config['credentials']['usernames'][st.session_state["username"]]['password'] = new_password_hash

    #                 with open('config.yaml', 'w') as file:
    #                     yaml.dump(config, file, default_flow_style=False)
    #             else:
    #                 st.info('No action performed')
    #         except Exception as e:
    #             st.error(e)
    #     else:
...
#                 # Return to prevent the except block from being executed, even if there is an error updating the config.yaml file.
#                 return
